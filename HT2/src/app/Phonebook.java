...
public class Phonebook {
    ...
    public boolean deleteTelephoneNumber(Person person, String numberId) {
        if ((numberId != null) && (!numberId.equals("null"))) {
            int filtered_id = Integer.parseInt(numberId);
            ...
            if (affected_rows > 0) {
                ...
                person.getPhones().remove(numberId);
                return true;
            } else {
                return false;
            }
    ...
    // Удаление записи о телефоне.
    public boolean editTelephoneNumber(Person person, String phoneNumber, String numberId) {
        if ((numberId != null) && (!numberId.equals("null"))) {
            if(person.getPhones().containsValue(phoneNumber)){
                return false;
            }
            int filtered_id = Integer.parseInt(numberId);

            Integer affected_rows = this.db.changeDBData("UPDATE `phone` SET `number` = '" + phoneNumber + "' WHERE `id` = " + filtered_id);

            // Если обновление прошло успешно...
            if (affected_rows > 0) {
                // Обновляем запись о телефоне в общем списке.
                person.getPhones().put(numberId, phoneNumber);
                return true;
            } else {
                return false;
            }
    ...
    // Удаление записи о телефоне.
    public boolean addTelephoneNumber(Person person, String phoneNumber) {
        if ((phoneNumber != null) && (!phoneNumber.equals("null"))) {
            ...
            Integer affected_rows = this.db.changeDBData("INSERT INTO `phone` (`owner`, `number`) VALUES ('" + person.getId() + "', '" + phoneNumber + "')");

            // Если добавление прошло успешно...
            if (affected_rows > 0) {
                // Добавляем запись о телефоне в общий список.

                person.getPhones().put(this.db.getLastInsertId().toString(), phoneNumber);
                return true;
            } else {
                return false;
            }
