...
<form action="<%=request.getContextPath()%>/" method="post">
    ...
    <table align="center" border="1" width="50%">
        ...
        <tr>
            ...
            <td>
                <%
                    for (Map.Entry<String, String> pair : person.getPhones().entrySet()) {
                %>
                    <%=pair.getValue()%>&emsp;
                <a href="<%=request.getContextPath()%>/ManagePhoneServlet/?action=editNumber&personId=<%=person.getId()%>&numberId=<%=pair.getKey()%>">Редактировать</a>&emsp;
                <a href="<%=request.getContextPath()%>/ManagePhoneServlet/?action=deleteNumber&personId=<%=person.getId()%>&numberId=<%=pair.getKey()%>">Удалить</a>
                <br/>
                <%
                    }
                %>
                <a href="<%=request.getContextPath()%>/ManagePhoneServlet/?action=addNumber&personId=<%=person.getId()%>">Добавить</a>
            </td>
        </tr>
        <tr>
